...
{
    public class GetAllCasePartiesHandlerTests
    {
        private readonly Mock<IUnitOfWork> _mockUnitOfWork;
        private readonly IMapper _mapper;
        private readonly GetAllCasePartiesHandler _handler;

        public GetAllCasePartiesHandlerTests()
        {
            ...
            _handler = new GetAllCasePartiesHandler(_mockUnitOfWork.Object, _mapper);
        }

        [Fact]
        public async Task Should_Return_All_CaseParties()
        {
            // Arrange
            var parties = new List<DomainEntities.CaseParty>
            {
                new DomainEntities.CaseParty { Id = 1, Name = "Party 1", CaseId = 1 },
                new DomainEntities.CaseParty { Id = 2, Name = "Party 2", CaseId = 1 }
            };

            _mockUnitOfWork.Setup(u => u.CasePartyRepository.GetAllAsync())
                .ReturnsAsync(parties);

            var query = new GetAllCasePartiesQuery();
            ...
            _mockUnitOfWork.Verify(u => u.CasePartyRepository.GetAllAsync(), Times.Once);
        }
    }
}
